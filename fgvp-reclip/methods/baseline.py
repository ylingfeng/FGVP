...
class Baseline(RefMethod):
    ...
    @overrides
    def execute(self, caption: str, env: "Environment", caption_bank: List[str]=[]) -> Dict[str, Any]:
        chunk_texts = self.get_chunk_texts(caption)
        probs = env.filter(caption, area_threshold = self.box_area_threshold, softmax=True, caption_bank=caption_bank)
        if self.args.baseline_head:
            probs2 = env.filter(chunk_texts[0], area_threshold = self.box_area_threshold, softmax=True, caption_bank=caption_bank)
